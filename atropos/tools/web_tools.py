...
_summarizer_client: Optional[AsyncOpenAI] = None
...
def _get_summarizer_client() -> AsyncOpenAI:
    global _summarizer_client

    if _summarizer_client is not None:
...
async def process_content_with_llm(
    ...
    try:
        ...
        # If we don't have an OpenRouter key, skip processing rather than failing.
        if not os.getenv("OPENROUTER_API_KEY"):
            print("🔑 OPENROUTER_API_KEY not set; skipping LLM processing")
            return None
...
def check_firecrawl_api_key() -> bool:
    ...
    """
    return bool(os.getenv("FIRECRAWL_API_KEY")) and Firecrawl is not None
...
# =============================================================================
# Atropos-Agent Tool wrappers (Hermes compatibility)
# =============================================================================

from .base import Tool, ToolResult, ToolSchema  # noqa: E402


def _tool_result_from_json(output: str) -> ToolResult:
    try:
    ...
    if isinstance(data, dict):
        if data.get("success") is False:
            err = data.get("error") or data.get("message") or "Tool failed"
            return ToolResult(success=False, output=output, error=str(err))
        if "error" in data and data.get("error"):
            return ToolResult(success=False, output=output, error=str(data["error"]))
...
class WebSearchTool(Tool):
    ...
    def schema(self) -> ToolSchema:
        return ToolSchema(
            name="web_search",
            description=(
                "Search the web for information on any topic. Returns up to 5 relevant results with titles and URLs."
            ),
            parameters={
    ...
    def is_available(self) -> tuple[bool, str | None]:
        if Firecrawl is None:
            return False, "firecrawl package not installed"
        if not os.getenv("FIRECRAWL_API_KEY"):
            return False, "FIRECRAWL_API_KEY not set"
        return True, None

    async def execute(self, query: str) -> ToolResult:
        output = web_search_tool(query, limit=5)
        return _tool_result_from_json(output)
...
class WebExtractTool(Tool):
    ...
    def schema(self) -> ToolSchema:
        return ToolSchema(
            name="web_extract",
            description=(
                "Extract and read the full content from specific web page URLs. Returns excerpts and key points."
            ),
            parameters={
                "urls": {
                    "type": "array",
    ...
    async def execute(self, urls: List[str]) -> ToolResult:
        ...
        output = await web_extract_tool(safe_urls, format="markdown", use_llm_processing=True)
        return _tool_result_from_json(output)
...
class WebCrawlTool(Tool):
    ...
    def schema(self) -> ToolSchema:
        return ToolSchema(
            name="web_crawl",
            description="Crawl a website and extract relevant content across pages.",
            parameters={
                "url": {"type": "string", "description": "The base URL to crawl (can include or exclude https://)"},
                "instructions": {
                    "type": "string",
                    "description": "Specific instructions for what to crawl/extract (optional).",
                },
            },
            required=["url"],
            external=True,
    ...
    async def execute(self, url: str, instructions: Optional[str] = None) -> ToolResult:
        output = await web_crawl_tool(url, instructions=instructions, depth="basic", use_llm_processing=True)
        return _tool_result_from_json(output)
