...
def _tool_result_from_json(output: str) -> ToolResult:
    ...
    if isinstance(data, dict) and "success" in data:
        ...
        if not ok:
            err = data.get("error") or "Vision analysis failed"
...
class VisionAnalyzeTool(Tool):
    ...
    def schema(self) -> ToolSchema:
        return ToolSchema(
            name="vision_analyze",
            description=(
                "Analyze images from URLs using AI vision. Provides a comprehensive description and answers a question."
            ),
            parameters={
                "image_url": {
                    "type": "string",
    ...
    async def execute(self, image_url: str, question: str) -> ToolResult:
        full_prompt = (
            "Fully describe and explain everything about this image, then answer the following question:\n\n"
            f"{question}"
        )
        output = await vision_analyze_tool(image_url=image_url, user_prompt=full_prompt, model=DEFAULT_VISION_MODEL)
