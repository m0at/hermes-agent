# NixOS module for Hermes Agent
# Usage: services.hermes-agent.enable = true;
flake:
{ config, lib, pkgs, ... }:
...
in
{
  options.services.hermes-agent = {
    ...
    environmentFile = lib.mkOption {
      ...
      description = ''
        ...
        Should contain lines like OPENROUTER_API_KEY=sk-...
      '';
    };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.hermes-agent-setup = {
      ...
      # Needs network for pip/npm installs
    ...
    systemd.services.hermes-agent-gateway = lib.mkIf cfg.gateway.enable {
      ...
      serviceConfig = {
        ...
        Environment = [
          "HOME=${cfg.stateDir}"
          "HERMES_HOME=${cfg.stateDir}/.hermes"
          "PATH=${lib.makeBinPath (with pkgs; [ python311 nodejs_20 ripgrep git openssh ])}:${cfg.stateDir}/app/.venv/bin"
        ];
